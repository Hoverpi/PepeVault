    ...
    view! {
        div(class="min-h-screen flex items-center justify-center bg-base-200 px-4") {
            div(class="w-full max-w-md bg-base-100 p-6 rounded-xl shadow-lg") {
                h1(class="text-2xl font-bold text-center mb-4") { "Login" }
                p(class="text-sm text-muted text-center mb-6") {
                    "Welcome back — sign in to continue to PepeVault"
                }
                ...
                form(class="space-y-4", action="#", method="post") {
                    ...
                    div(class="flex flex-col gap-2 mt-2") {
                        a(class="btn btn-ghost w-full", href="/signup", role="button") { "Sign up" }
